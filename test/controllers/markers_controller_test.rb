...
class MarkersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create marker" do
    assert_difference("Marker.count") do
      post markers_url, params: { marker: { description: @marker.description, name: @marker.name, status: @marker.status } }
    end
  ...
  test "should update marker" do
    patch marker_url(@marker), params: { marker: { description: @marker.description, name: @marker.name, status: @marker.status } }
